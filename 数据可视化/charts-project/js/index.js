...
(function() {
  ...
  $.each(hotData, function (i, e) {
    // console.log( e);
    supHTML += `<li> <span> ${e.city}</span><span>${e.sales}<s class="${e.flag ? "icon-up " : "icon-down"}"></s></span></li>`;
  })
  // var supHTML = "";
  ...
  var timer = setInterval(function () {
    ...
    // 声明遍量
    var subHTML = ''
    //  遍历对象根据点击当前li的索引来选中对应的城市遍历里面的对象
    $.each(hotData[index].brands, function (i, e) {
      subHTML += `<li> <span> ${e.name}</span><span>${e.num}<s class="${e.flag ? "icon-up " : "icon-down"}"></s></span></li>`;
    })
    $(".sub").html(subHTML);
  }, 2000)
